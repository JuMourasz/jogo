...
<head>
  ...
  <title>O Caminho Proibido - Fase 3: Caverna das Sombras</title>
  <style>
...
#select-phases {
  right: 10px;
    ...
    #game-container {
      ...
      width: 800px;
      height: 600px;
      background: url("https://i.ibb.co/67DKCVHQ/de7120fd518fc6ed6cf00f0ff9b3f8f1.jpg") no-repeat center/cover;
      overflow: hidden;
...
<body>
  <div id="game-container" tabindex="0" aria-label="Área do jogo - Fase 3">
...
<button id="select-phases" aria-label="Selecionar fases">Selecionar Fases</button>
    ...
    <div class="enemy" style="top: 100px; left: 150px;" aria-label="Inimigo normal"></div>
    <div class="enemy" style="top: 200px; left: 300px;" aria-label="Inimigo normal"></div>
    <div class="enemy" style="top: 400px; left: 200px;" aria-label="Inimigo normal"></div>
    <div class="enemy" style="top: 150px; left: 500px;" aria-label="Inimigo normal"></div>
    <div class="enemy patrol" style="top: 300px; left: 100px;" aria-label="Inimigo patrulheiro"></div>
    <div class="enemy elite" style="top: 250px; left: 650px;" aria-label="Elite perto da porta"></div>

    <div class="potion" style="top: 150px; left: 350px;" aria-label="Poção"></div>
    <div class="potion" style="top: 450px; left: 600px;" aria-label="Poção"></div>

    <div class="wall" style="width: 80px; height: 100px; top: 250px; left: 400px;"></div>
    <div class="wall1" style="width: 45px; height: 70px; top: 100px; left: 550px;"></div>
    <div class="wall2" style="width: 150px; height: 80px; top: 400px; left: 300px;"></div>
    <div class="wall3" style="width: 70px; height: 90px; top: 50px; left: 200px;"></div>
    <div class="wall4" style="width: 100px; height: 100px; top: 350px; left: 100px;"></div>

    <div class="spike" style="top: 180px; left: 280px;" aria-label="Spike perigoso"></div>
  ...
  <script>
    (function(){
      ...
      gameState.enemies = enemies.map(enemy => ({
        ...
        speed: enemy.classList.contains('elite') ? 1. : (enemy.classList.contains('patrol') ? 1.0 : gameState.enemySpeed)
      }));
...
collectSound.play();
                ...
                gameState.buffTimer = 300; 
                const buffMsg = potion.type === 'speed' ? 'velocidade' : 'força';
                messageBox.textContent = `Poção coletada! Buff de ${buffMsg} por 5s.`;
                setTimeout(() => {
...
victorySound.play();
    messageBox.textContent = "Fase 3 Concluída! Parabéns, você venceu e está indo para a Fase 4!";
    const completedPhases = JSON.parse(localStorage.getItem('completedPhases')) || [];
if (!completedPhases.includes('phase3')) {
  completedPhases.push('phase3');
  localStorage.setItem('completedPhases', JSON.stringify(completedPhases));
}
    setTimeout(() => {
      window.location.href = "{{ route('introfase4') }}";;
    }, 2000);
}
